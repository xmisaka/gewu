...
- **物品列表** —— 搜索、概览、分类筛选三段式。概览给三个数：在库 / 总价值 / 即将到期。
  右上角「列表 / 位置」是视图切换器，位置视图由此进入，不额外占底部标签。
- **物品详情** —— 持有成本卡用品牌浅底，是全 App 唯一带情绪回报的信息，所以给它独立成块。
  价格或购买日期任一缺失时**整卡隐藏**，绝不留 `¥0.00 / 天` 这种占位 —— 那会让人以为东西是白来的。
- **连续录入** —— 主按钮是「保存并录下一件」而不是「完成」，视觉权重也更大，顶部留「第 3 件」的进度。
  批量录入是 V1 成败的关键，界面要推着人往前走，而不是引导他退出。
...
- **位置视图** —— 两级结构：柜子 → 格位。柜子卡片上的那组小色块就是**格位占用示意**，
  填充越浅表示空位越多，一眼看出哪个柜子塞满了。物品可以只挂到柜子，也可以精确到某一格。
- **柜内格位** —— 点进柜子后按格位分组。「找东西」场景的主战场：先想到柜子，再扫一眼格位，
  不用记编号，也不必先搜索。
- **相册** —— 照片按年月倒序分组，统一正方形裁切。加载的是压缩图而非原图 ——
  盘点、理赔、出手时用得上，同时不让相册把手机撑爆。
...
- **到期** —— 只做到期一件事，按严重程度分三段：已过期 / 30 天内到期 / 更远，
  最紧急的排最前。**不做系统推送**：打开这一页即可查看，系统级提醒留给后续版本。
- **我的** —— 备份状态置顶且用品牌浅底。纯本地方案里这是唯一的救命绳，
  不能藏在三级菜单里。下方按「数据通道 / 整理 / 外观」分组。
- **主题** —— 四套浅色任选（素笺 / 靛青 / 青瓷 / 朱砂）。深色档「玄夜」**不进选择列表**：
  它是系统深色时的接管档，自动生效，而用户的浅色选择会留着 —— 白天切回来还是那套。

> 以上是界面设计稿，源文件 `docs/screens.html`，已与 v1.2.1 的实现逐屏核对。
> 图由无头 Chrome 渲染该 HTML 产出，改设计后重跑一次即可更新。
